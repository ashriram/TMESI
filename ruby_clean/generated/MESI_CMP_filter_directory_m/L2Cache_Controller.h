...
/** \brief Broadcast a Write Filter lookup request*/
  void a_checkL1WriteFiltersExceptRequestor(const Address& addr);
/** \brief Broadcast a Read + Write Filter lookup request*/
  void a_checkL1ReadWriteFiltersExceptRequestor(const Address& addr);
/** \brief Broadcast a Write Filter lookup request*/
  void a_checkNackerL1WriteFiltersExceptRequestor(const Address& addr);
/** \brief Broadcast a Read + Write Filter lookup request*/
  void a_checkNackerL1ReadWriteFiltersExceptRequestor(const Address& addr);
/** \brief Send data from cache to reqeustor*/
  void f_sendDataToGetSRequestor(const Address& addr);
/** \brief Send data from cache to reqeustor*/
  void f_sendExclusiveDataToGetSRequestor(const Address& addr);
/** \brief Send data from cache to reqeustor*/
  void f_sendDataToGetXRequestor(const Address& addr);
/** \brief Send data from cache to reqeustor*/
  void f_sendDataToRequestor(const Address& addr);
/** \brief Send data from cache to reqeustor*/
  void f_sendExclusiveDataToRequestor(const Address& addr);
/** \brief Send ACKs to requestor*/
  void f_sendAccumulatedAckToRequestor(const Address& addr);
/** \brief invalidate sharers for request, requestor is sharer*/
  void fwm_sendFwdInvEscapeToSharersMinusRequestor(const Address& addr);
/** \brief Profiles the requestor*/
  void f_profileRequestor(const Address& addr);
/** \brief Mark an L2 block as transactional*/
  void f_markBlockTransIfTrans(const Address& addr);
...
/** \brief Sets the physical address field of the TBE*/
  void i_setTBEPhysicalAddress(const Address& addr);
...
/** \brief Send NACK to upgrader*/
  void ts_sendInvNackToUpgrader(const Address& addr);
