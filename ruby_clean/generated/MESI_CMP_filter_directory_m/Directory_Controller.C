/** \file Directory.C
  ...
  * Created by slicc definition of Module "MESI_CMP_filter_directory protocol"
  */
...
void Directory_Controller::qf_queueMemoryFetchRequest(const Address& addr)
{
  DEBUG_MSG(GENERATED_COMP, HighPrio,"executing");
  {
    ...
    assert(in_msg_ptr != NULL);
    {
      ...
      DEBUG_SLICC(MedPrio, "../protocols/MESI_CMP_filter_directory_m-mem.sm:222: ", out_msg);
;
...
void Directory_Controller::qw_queueMemoryWBRequest(const Address& addr)
{
  DEBUG_MSG(GENERATED_COMP, HighPrio,"executing");
  {
    ...
    assert(in_msg_ptr != NULL);
    {
      ...
      (out_msg).m_DataBlk = ((*in_msg_ptr)).m_DataBlk;
      (out_msg).m_MessageSize = ((*in_msg_ptr)).m_MessageSize;
      DEBUG_SLICC(MedPrio, "../protocols/MESI_CMP_filter_directory_m-mem.sm:238: ", out_msg);
;
      ((*(m_chip_ptr->m_Directory_memBuffer_vec[m_version]))).enqueue(out_msg, 1);
    }
  }
...
void Directory_Controller::m_writeDataToMemory(const Address& addr)
{
  DEBUG_MSG(GENERATED_COMP, HighPrio,"executing");
  {
    ...
    DEBUG_SLICC(MedPrio, "../protocols/MESI_CMP_filter_directory_m-mem.sm:246: ", ((*in_msg_ptr)).m_Address);
;
    DEBUG_SLICC(MedPrio, "../protocols/MESI_CMP_filter_directory_m-mem.sm:247: ", ((*in_msg_ptr)).m_DataBlk);
;
  }
}
