...
Chip::Chip(NodeID id, Network* net_ptr):AbstractChip(id, net_ptr)
{
  ...
  // L1Cache_triggerQueue
  m_L1Cache_triggerQueue_vec.setSize(RubyConfig::numberOfL1CachePerChip(m_id));
  for (int i = 0; i < RubyConfig::numberOfL1CachePerChip(m_id); i++)  {
    m_L1Cache_triggerQueue_vec[i] = new MessageBuffer(this);

    assert(m_L1Cache_triggerQueue_vec[i] != NULL);
    m_L1Cache_triggerQueue_vec[i]->setOrdering(false);
    if (FINITE_BUFFERING) {
      m_L1Cache_triggerQueue_vec[i]->setSize(PROCESSOR_BUFFER_SIZE);
    }
  }
  ...
  for (int i = 0; i < RubyConfig::numberOfDirectoryPerChip(m_id); i++)  {
    m_Directory_memBuffer_vec[i] = new MemoryControl(this, i);
...
Chip::~Chip()
{
  ...
  for (int i = 0; i < RubyConfig::numberOfL1CachePerChip(m_id); i++)  {
    delete m_L1Cache_triggerQueue_vec[i];
  }
  for (int i = 0; i < RubyConfig::numberOfDirectoryPerChip(m_id); i++)  {
...
void Chip::printConfig(ostream& out)
{
  ...
  out << "\nL1Cache_triggerQueue numberPerChip: " << RubyConfig::numberOfL1CachePerChip() << endl;
  m_L1Cache_triggerQueue_vec[0]->printConfig(out);
