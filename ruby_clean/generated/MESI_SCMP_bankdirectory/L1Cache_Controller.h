...
public:
  ...
  GenericMachineType L1Cache_getNondirectHitMachType(Address param_addr, MachineID param_sender);
  // Actions
/** \brief Issue GETS*/
...
/** \brief send invalidate ack to requestor (could be L2 or L1)*/
  void e_sendAckReadToRequestor(const Address& addr);
/** \brief send invalidate ack to requestor (could be L2 or L1)*/
  void e_sendAckThreatToRequestor(const Address& addr);
/** \brief send data to the L2 cache*/
...
/** \brief send unblock threat to the L2 cache from IM*/
  void j_sendUnblockIM(const Address& addr);
...
/** \brief Check for Conflict due to External Fwd msg.*/
  void o_checkForConflict(const Address& addr);
/** \brief Abort Transaction due to external invalidation or L2 eviction*/
  void o_checkforTxAbort(const Address& addr);
...
/** \brief Notify sequencer the load completed.*/
  void x_external_load_hit(const Address& addr);
...
/** \brief Notify sequencer that store completed.*/
  void xx_external_store_hit(const Address& addr);
...
/** \brief Allocate TBE and set Trans bit*/
  void s_setTBETrans(const Address& addr);
/** \brief Allocate TBE and set Threat  bit*/
  void s_setThreatBit(const Address& addr);
