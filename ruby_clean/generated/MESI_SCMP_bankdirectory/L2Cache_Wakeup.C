      ...
      if ((((*(m_chip_ptr->m_L2Cache_responseToL2Cache_vec[m_version]))).isReady())) {
        {
          ...
          assert(in_msg_ptr != NULL);
            ...
            cerr << "Runtime Error at ../protocols/MESI_SCMP_bankdirectory-L2cache.sm:314, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
...
;
          if (((machineIDToMachineType(((*in_msg_ptr)).m_Sender)) == MachineType_L1Cache)) {
            if ((((*in_msg_ptr)).m_Type == CoherenceResponseType_DATA)) {
              if ((((L2Cache_getState(((*in_msg_ptr)).m_Address)) == L2Cache_State_S_I) && ((((((*(m_chip_ptr->m_L2Cache_L2_TBEs_vec[m_version]))).lookup(((*in_msg_ptr)).m_Address))).m_pendingAcks - ((*in_msg_ptr)).m_AckCount) == (0)))) {
                                {
                  Address addr = ((*in_msg_ptr)).m_Address;
                  TransitionResult result = doTransition(L2Cache_Event_WB_Data_All, L2Cache_getState(addr), addr);
...
;
                                ...
                                cerr << "Runtime Error at ../protocols/MESI_SCMP_bankdirectory-L2cache.sm:334, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("unknown message type") << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
...
;
                                ...
                                cerr << "Runtime Error at ../protocols/MESI_SCMP_bankdirectory-L2cache.sm:343, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("unknown message type") << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
      ...
      if ((((*(m_chip_ptr->m_L2Cache_L1RequestToL2Cache_vec[m_version]))).isReady())) {
        {
          ...
          assert(in_msg_ptr != NULL);
          DEBUG_SLICC(MedPrio, "../protocols/MESI_SCMP_bankdirectory-L2cache.sm:354: ", ((*in_msg_ptr)).m_Address);
;
          DEBUG_SLICC(MedPrio, "../protocols/MESI_SCMP_bankdirectory-L2cache.sm:355: ", m_chip_ptr->getID());
;
          DEBUG_SLICC(MedPrio, "../protocols/MESI_SCMP_bankdirectory-L2cache.sm:356: ", (L2Cache_getState(((*in_msg_ptr)).m_Address)));
;
          DEBUG_SLICC(MedPrio, "../protocols/MESI_SCMP_bankdirectory-L2cache.sm:357: ", ((*in_msg_ptr)).m_Requestor);
;
          DEBUG_SLICC(MedPrio, "../protocols/MESI_SCMP_bankdirectory-L2cache.sm:358: ", ((*in_msg_ptr)).m_Type);
;
          DEBUG_SLICC(MedPrio, "../protocols/MESI_SCMP_bankdirectory-L2cache.sm:359: ", ((*in_msg_ptr)).m_Destination);
;
                    if (ASSERT_FLAG && !(((machineIDToMachineType(((*in_msg_ptr)).m_Requestor)) == MachineType_L1Cache))) {
            cerr << "Runtime Error at ../protocols/MESI_SCMP_bankdirectory-L2cache.sm:360, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();

          }
;
                    if (ASSERT_FLAG && !(((((*in_msg_ptr)).m_Destination).isElement(m_machineID)))) {
            cerr << "Runtime Error at ../protocols/MESI_SCMP_bankdirectory-L2cache.sm:361, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
      ...
      if ((((*(m_chip_ptr->m_L2Cache_unblockToL2Cache_vec[m_version]))).isReady())) {
        {
          ...
          assert(in_msg_ptr != NULL);
                    if (ASSERT_FLAG && !(((((*in_msg_ptr)).m_Destination).isElement(m_machineID)))) {
            cerr << "Runtime Error at ../protocols/MESI_SCMP_bankdirectory-L2cache.sm:385, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << "assert failure" << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
...
;
              ...
              if ((((*in_msg_ptr)).m_Type == CoherenceResponseType_UNBLOCK_DATA)) {
                                {
                  Address addr = ((*in_msg_ptr)).m_Address;
                  TransitionResult result = doTransition(L2Cache_Event_Unblock_Data, L2Cache_getState(addr), addr);
                  if (result == TransitionResult_Valid) {
                    counter++;
                    continue; // Check the first port again
                  }
...
;
          ...
          if ((((*in_msg_ptr)).m_Type == CoherenceResponseType_UNBLOCK_DATA_THREAT)) {
                        {
              Address addr = ((*in_msg_ptr)).m_Address;
              TransitionResult result = doTransition(L2Cache_Event_Unblock_Data_Threat, L2Cache_getState(addr), addr);
              if (result == TransitionResult_Valid) {
                counter++;
                continue; // Check the first port again
              }
...
;
          ...
          if ((((*in_msg_ptr)).m_Type == CoherenceResponseType_UNBLOCK_THREAT)) {
                        {
              Address addr = ((*in_msg_ptr)).m_Address;
              TransitionResult result = doTransition(L2Cache_Event_Unblock_Threat, L2Cache_getState(addr), addr);
              if (result == TransitionResult_Valid) {
                counter++;
                continue; // Check the first port again
              }
...
;
                        ...
                        cerr << "Runtime Error at ../protocols/MESI_SCMP_bankdirectory-L2cache.sm:398, Ruby Time: " << g_eventQueue_ptr->getTime() << ": " << ("unknown unblock message") << ", PID: " << getpid() << endl;
char c; cerr << "press return to continue." << endl; cin.get(c); abort();
